...
function agregardatosI(id_pregunta_f, su_pregunta, respuesta, usuario_id){
    cadena = "id_pregunta_f=" + id_pregunta_f +
    "&su_pregunta=" + su_pregunta +
    "&respuesta=" + respuesta +
    "&usuario_id=" + usuario_id;
    ...
    accion = "insertar";
    mensaje_si = "Pregunta frecuente agregada con éxito";
    mensaje_no= "Error de registro";
...
function preguntarSiNoI(id_pregunta_f) {
    var opcion = confirm("¿Esta seguro de eliminar el registro?");
    if (opcion == true) {
        alert("El registro será eliminado.");
        eliminarDatosI(id_pregunta_f);
    } else {
        alert("El proceso de eliminación del registro ha sido cancelado.");
    }
}
...
function eliminarDatosI(id_pregunta_f) {
    cadena = "id_pregunta_f=" + id_pregunta_f;
...
function a_ajax_i(cadena, accion, mensaje_si, mensaje_no){
    $.ajax({
        ...
        success: function (r){
            if (r > 0) {
            $('#tabla').load('../vista/componentes/vista_preguntas_frecuentes_id.php?id='+r);
                alert(mensaje_si);
            } else {
                alert(mensaje_no);
            }
