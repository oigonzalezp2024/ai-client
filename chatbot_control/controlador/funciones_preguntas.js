...
function agregardatosI(id_pregunta, su_pregunta, respuesta, persona_id){
    cadena = "id_pregunta=" + id_pregunta +
    "&su_pregunta=" + su_pregunta +
    "&respuesta=" + respuesta +
    "&persona_id=" + persona_id;
    ...
    accion = "insertar";
    mensaje_si = "Pregunta agregada con exito";
    mensaje_no= "Error de registro";
...
function preguntarSiNoI(id_pregunta) {
    var opcion = confirm("¿Esta seguro de eliminar el registro?");
    if (opcion == true) {
        alert("El registro será eliminado.");
        eliminarDatosI(id_pregunta);
    } else {
        alert("El proceso de eliminación del registro ha sido cancelado.");
    }
}
...
function eliminarDatosI(id_pregunta) {
    cadena = "id_pregunta=" + id_pregunta;
...
function a_ajax_i(cadena, accion, mensaje_si, mensaje_no){
    $.ajax({
        ...
        success: function (r){
            if (r > 0) {
            $('#tabla').load('../vista/componentes/vista_preguntas_id.php?id='+r);
                alert(mensaje_si);
            } else {
                alert(mensaje_no);
            }
